...
public class OPDSActivity extends Activity {
    private ArrayList<Item> names = new ArrayList<>();
    private ArrayList<BookInfo> books = new ArrayList<>();

    private ArrayList<String> list_book_steps = new ArrayList<>();
    private ArrayList<String> list_path_steps = new ArrayList<>();
    //private String_Search_Tag search_site;
    // db
    private int opdsID;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if(!app.haveNetworkConnection(getApplicationContext())){
        ...
        countDownload = 0;
        setContentView(R.layout.layout_opds);
        ...
        test = this;
        scrollStep = Integer.parseInt(prefs.getString("scrollPerc","10"));
        ...
        opdsID = Integer.parseInt(data.getExtras().getString("opds_id"));
        UtilOPDS utilOPDS = new UtilOPDS(app.getBaseContext());

        URL_START_LINK = utilOPDS.getUrlOPDS(opdsID);
        ...
        ImageButton back_btn = (ImageButton) findViewById(R.id.results_btn_exit);
    ...
    private void showBookInfo(int count_book){
        ...
        dialog.setContentView(R.layout.dialog_layout_bookinfo);

        ((ImageView) dialog.findViewById(R.id.book_icon)).setImageResource(R.drawable.ci_bookinfo);
        ...
        if(books.get(count_book).book_authors != null){
            ...
            if(context != null) {
                final ArrayAdapter<String> lvAdapter = new ArrayAdapter<>(context, R.layout.item_bookinfo, authors);
    ...
    private class DownloadTask extends AsyncTask<String, Integer, String> {
        ...
        DownloadTask(Context context, String namefile, String url) {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            countDownload++;
            dowloadTitleCount(String.valueOf(((TextView) findViewById(R.id.results_title)).getText()));
                    ...
                    if (eBook.isOk) {
                        ...
                        if (eBook.authors.size() > 0) {
                            ...
                            if(!newDir.isDirectory()){
                                if (!(new LocalFile(context)).createDir(fileMoveDir)){
                                    return;
                                }
                            }
                            if (eBook.sequenceName != null && eBook.sequenceName.length() != 0) {
                                ...
                                if(!newDir.isDirectory()){
                                    if ((new LocalFile(context)).createDir(fileMoveDir + sequenceN + File.separator)){
                        ...
                        (new LocalFile(context)).moveFile(OPDS_Path_Download + File.separator + name_file, fileMoveDir + File.separator + name_file);
                    }
                }
            }
        }
    ...
    private boolean Entry_Tag(XmlPullParser xpp, ArrayList<Item> names_tmp, ArrayList<BookInfo> books_tmp){
        ...
        ArrayList<String> entry_authors_list = new ArrayList<>();
        ArrayList<BookLink> entry_books_link = new ArrayList<>();
        ArrayList<String_Search_Tag> tmp_list_link = new ArrayList<>();
        String url_pref;
    ...
    private class OPDS_adapter extends BaseAdapter {
        ...
        OPDS_adapter(Context context, ArrayList<Item> arr) {
            if (arr != null) {
                data = arr;
            }
            this.context = context;
        ...
        public View getView(int i, View someView, ViewGroup arg2) {
            ...
            if (someView == null) {
                someView = inflater.inflate(R.layout.item_opds, arg2, false);
            }
            if (someView == null) {
                return null;
            }
    ...
    public class DownloadImageTask extends AsyncTask<Void, Integer, Bitmap>{
        ...
        DownloadImageTask(ImageView source, String url) {
            this.img = source;
            this.urlImage = url;
        }
    ...
    private class LoadOPDS extends AsyncTask<Void, For_Load_OPDS, Void> {
        ...
        LoadOPDS(For_Load_OPDS for_load_opds) {
    ...
    private Request_Load_OPDS load_opds_catalog(String URL_LINK, int opdsID, String flag){
        ...
        ArrayList<Item> names_tmp = new ArrayList<>();
        ArrayList<BookInfo> books_tmp = new ArrayList<>();
