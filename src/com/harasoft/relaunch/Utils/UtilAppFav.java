...
import com.harasoft.relaunch.DB.ListAppFav;
 ...
 * Работа со списком избранных приложений
 */
public class UtilAppFav {
    private Context context;
    private int appMax;

    public UtilAppFav(Context context) {
    ...
    public ArrayList<String> getList(){
        ...
        Cursor c = db.query(ListAppFav.TABLE_NAME, null, null, null, null, null, null);
        // определяем номера столбцов по имени в выборке
        int ureColIndex = c.getColumnIndex(ListAppFav.COLUMN_APP_PACKAGE);
    ...
    public void addAppFav(String appPackage){
        // если уже есть, то удаляем и затем вставим новое
        if (isAppFav(appPackage)) {
    ...
    private void delItemsDB (int countItems) {
        ...
        if (c.moveToFirst()) {
            do {
                db.delete(ListAppFav.TABLE_NAME, ListAppFav.COLUMN_APP_PACKAGE + " = ?", new String[]{c.getString(ureColIndex)});
                count++;
                // переход на следующую строку
                // а если следующей нет (текущая - последняя), то false - выходим из цикла
    ...
    public boolean isAppFav(String appPackage){
    ...
    public void delAppFav(String appPackage) {
        ...
        db.delete(ListAppFav.TABLE_NAME, ListAppFav.COLUMN_APP_PACKAGE + " = ?", new String[]{appPackage});
        closeDb(DbHelper, db, null);
    }
    public void setListAppFav(ArrayList<UtilApp.Info> listAppFav) {
        resetDb();
        int count = appMax;
    ...
    private void addApp(String appPackage){
        ...
        cv.put(ListAppFav.COLUMN_APP_PACKAGE, appPackage);

        // вставляем запись и получаем ее ID
        db.insert(ListAppFav.TABLE_NAME, null, cv);
    ...
    public void resetDb() {
        ...
        // открываем базу на запись
        SQLiteDatabase db = DbHelper.getWritableDatabase();
        db.execSQL("delete from " + ListAppFav.TABLE_NAME);
